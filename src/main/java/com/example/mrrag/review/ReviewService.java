...
import com.example.mrrag.review.pipeline.RagPipeline;
 ...
 *   <li>Enrich groups with context snippets ({@link ChangeGroupEnrichmentPort})</li>
 *   <li>Run RAG pipeline ({@link RagPipeline}) → classify, find classes/nodes,
...
// 2026-04-08 16:54:41.783  INFO 5304 --- [onPool-worker-1] c.example.mrrag.graph.GraphBuilderImpl   : doBuildGraphFromSources: merged graph — 45172 nodes, 6032 edge-sources
        ...
        // --- RAG pipeline: classify → find classes/nodes → collect context → build representations ---
        log.info("Running RAG pipeline...");
        List<GroupRepresentation> representations = ragPipeline.run(
                groups, sourceGraph, targetGraph, sourceProvider.localProjectRoot().get(), targetProvider.localProjectRoot().get());
        log.info("RAG pipeline complete: {} representation(s)", representations.size());
